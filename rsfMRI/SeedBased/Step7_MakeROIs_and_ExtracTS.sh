...
for subID in $subID; do
	...
	ses=$(</projects/kg98/Thapa/cTBS_Study/6_RestingState/18_Final_ROIanalysis/ScriptsUsed/Sessions.txt)
		...
		#Step 6: Get initial affine transform that maps T1 to MNI.
		...
		#Step 7: Use the affine transform to get warp coefficients to convert T1 to MNI space.
		...
		#Step 8: Rename warpcoeffient file to subID and session.
		...
		#Step 9: Inverse the T1_to_MNI warp coefficients to MNI_to_T1 warp. You can use this to convert the PFC sphere to T1 space, as it is currently in MNI space.
		...
		#Step 10: Apply the MNI_to_T1 warp to PFC sphere as the PFC sphere is in MNI space. So, we need to convert it to T1 space.
		...
		#Step 11: Get matrix to convert T1 to func space.
		...
		#Step 12: Get PFC sphere that is in T1 space to func space.
		flirt -ref $fmriprep_dir/$subID/$subID'_ses-'$ses'_task-Rest'$ses'_space-MNI152NLin2009cAsym_variant-2PGSR_regressed-preproc_bold_smooth.nii' \
		...
		#Step 13: Binarise PFC sphere_space-func.
		...
		#Step 14: Use fslmeants, and the PFC sphere_space-func mask to extract the timeseries from the preproc_bold_smooth.nii file.
